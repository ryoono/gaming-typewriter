...
# ZOOM AMS-44 ASIO Driver の安定性を考慮し、4chで入力して1,2ch目を保存する
# 本当に2ch入力デバイスを使う場合は INPUT_CHANNELS = 2 に変更する
INPUT_CHANNELS = 4

# 保存するチャンネル番号
# 0: ch1, 1: ch2, 2: ch3, 3: ch4
SAVE_CHANNEL_INDICES = [0, 1]
...
POST_TRIGGER_SEC = 0.400    # 現物合わせで400msにした（だいたい300msくらいで収まる）
...
def save_trigger_audio(trigger_count, trigger_sample_index):
    ...
    # 4chで入力している場合、指定した2chだけ保存する
    if audio.ndim == 2:
        max_save_channel_index = max(SAVE_CHANNEL_INDICES)

        if max_save_channel_index >= audio.shape[1]:
            print("[ERROR] SAVE_CHANNEL_INDICES が入力チャンネル数を超えています。")
            print(f"        SAVE_CHANNEL_INDICES={SAVE_CHANNEL_INDICES}")
            print(f"        audio.shape={audio.shape}")
            return None

        audio_to_save = audio[:, SAVE_CHANNEL_INDICES]
    else:
        print("[ERROR] 2ch保存を行うには、音声データが2次元である必要があります。")
        print(f"        audio.shape={audio.shape}")
        return None
...
def main():
    if len(SAVE_CHANNEL_INDICES) != 2:
        raise ValueError("SAVE_CHANNEL_INDICES は2ch分を指定してください。")

    if min(SAVE_CHANNEL_INDICES) < 0:
        raise ValueError("SAVE_CHANNEL_INDICES は0以上にしてください。")

    if max(SAVE_CHANNEL_INDICES) >= INPUT_CHANNELS:
        raise ValueError("SAVE_CHANNEL_INDICES は INPUT_CHANNELS 未満にしてください。")
    ...
    print(f"Save ch     : {[ch + 1 for ch in SAVE_CHANNEL_INDICES]}")
